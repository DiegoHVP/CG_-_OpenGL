...
#define TEXTURA_DO_DISCO "logo.png"
/* #include <SOIL/SOIL.h> */


// GLuint textura_logo;  // logotipo
// A função carregaTexturaLogo e o uso de SOIL foram removidos porque png_texture_load é utilizado para carregar texturas neste código.
...
void desenhaMissil(float x, float y, float z) {
    ...
    gluCylinder(quad, 0.15, 0.15, 0.9, 12, 3);
    glTranslatef(0.0, 0.0, 0.9);
    glutSolidCone(0.15, 0.6, 12, 12);
    glPopMatrix();
...
void compoe_jato(void){
  ...
  /* cauda */
// Aplica logotipo SOMENTE no meio da cauda
glBindTexture(GL_TEXTURE_2D, textura_logo);
glBegin(GL_POLYGON); 
glTexCoord2f(0.0, 0.0); glVertex3fv(cauda[0]);
glTexCoord2f(1.0, 0.0); glVertex3fv(cauda[1]);
glTexCoord2f(1.0, 1.0); glVertex3fv(cauda[2]);
glTexCoord2f(0.0, 1.0); glVertex3fv(cauda[3]);
glEnd();

// Retorna à textura principal do avião
glBindTexture(GL_TEXTURE_2D, textura_aviao);
  ...
  // tag::desenho[]
  desenhaTurbina(); // Desenha a turbina
  desenhaMissil(-1.2, -0.3, -2.2); 
  desenhaMissil( 1.2, -0.3, -2.2);
  // end::desenho[]
...
void carregar_texturas(void){
  ...
  textura_logo  = png_texture_load(TEXTURA_DO_DISCO, NULL, NULL);
}
